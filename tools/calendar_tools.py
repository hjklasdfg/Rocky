...
def read_calendar(date: str | None = None, calendar_id: str = "all", credentials=None) -> dict:
    """Read events from Google Calendar for a specific date.

    Args:
        date: Date string in YYYY-MM-DD format. Defaults to today.
        calendar_id: Calendar ID. Default "all" aggregates events across every
            calendar the user has subscribed to (personal + shared + holidays).
            Pass "primary" to limit to the primary calendar only, or a specific
            calendar ID returned by list_calendars to target one calendar.

    Returns:
        dict with status and list of events. Each event includes `calendar`
        (the source calendar's display name) so the agent can disambiguate
        when events come from multiple calendars.
    """
    ...
    # "all" → every calendar in the user's calendarList (incl. holidays as
    # requested). Any other value → just that one calendar.
    if calendar_id == "all":
        cal_list = service.calendarList().list().execute().get("items", [])
        targets = [(c["id"], c.get("summary", c["id"])) for c in cal_list]
    else:
        targets = [(calendar_id, calendar_id)]

    events = []
    for cal_id, cal_name in targets:
        try:
            ...
            # Don't fail the whole call if one calendar errors (e.g. ACL change)
            continue
    ...
    # Aggregate sort across calendars by start time
