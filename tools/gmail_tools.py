...
def read_emails(max_results: int = 10, credentials=None, user_id=None) -> dict:
    """Read recent emails from Gmail INBOX (any category).

    Returns the most recent INBOX messages without category filtering — Gmail's
    `category:primary` filter is too strict for users whose mail mostly lands in
    Promotions/Updates/Social. Use search_emails with `is:unread` or other Gmail
    query syntax when the user wants a more specific slice.
